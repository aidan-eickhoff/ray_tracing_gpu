...
glm::vec3 renderRays(RenderState& state, std::span<const Ray> rays, int rayDepth)
{
    ...
    for (const auto& ray : rays) {
		state.time = state.sampler.next_1d();
...
Ray generateReflectionRay(Ray ray, HitInfo hitInfo)
{
	...
	glm::vec3 new_direction = ray.direction - 2.0f * glm::dot(ray.direction, hitInfo.normal) * hitInfo.normal;
    return Ray {origin, new_direction, 1.0f / new_direction};
}
...
Ray generatePassthroughRay(Ray ray, HitInfo hitInfo)
{
    ...
    return Ray {ray.origin + ray.direction * (ray.t + 1e-4f), ray.direction, ray.inv_d};
}
