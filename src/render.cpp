...
void renderImage(const Scene& scene, BVHInterface& bvh, const Features& features, const Trackball& camera, Screen& screen)
{
...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    std::vector<Ray> rays;
	glm::vec2 pos = (glm::vec2(pixel));
	glm::vec2 divisor = 2.0f / glm::vec2(screenResolution);
    for(uint32_t i = 0; i < numSamples; i++) {
		rays.push_back(camera.generateRay((pos + state.sampler.next_2d()) * divisor - 1.0f));
	}
    return rays;
}
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
	...
	glm::vec2 pos = (glm::vec2(pixel));
	glm::vec2 divisor = 2.0f / glm::vec2(screenResolution);
    for(float x = 0.0f; x < 1.0f; x+= 1.0f / (float)numSamples) {
		for(float y = 0.0f; y < 1.0f; y+= 1.0f / (float)numSamples) {
			rays.push_back(camera.generateRay((pos + glm::vec2(x, y) + (state.sampler.next_2d() / (float)numSamples)) * divisor - 1.0f));
		}
	}
	return rays;
}