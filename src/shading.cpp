...
glm::vec3 computeLambertianModel(RenderState& state, const glm::vec3& cameraDirection, const glm::vec3& lightDirection, const glm::vec3& lightColor, const HitInfo& hitInfo)
{
    ...
    return lightColor * sampleMaterialKd(state, hitInfo) * glm::dot(hitInfo.normal, lightDirection);
}
...
glm::vec3 computePhongModel(RenderState& state, const glm::vec3& cameraDirection, const glm::vec3& lightDirection, const glm::vec3& lightColor, const HitInfo& hitInfo)
{
	...
	return lightColor * hitInfo.material.ks * glm::pow(glm::dot(glm::normalize(glm::reflect(-lightDirection, hitInfo.normal)), cameraDirection), hitInfo.material.shininess);   
}
...
glm::vec3 computeBlinnPhongModel(RenderState& state, const glm::vec3& cameraDirection, const glm::vec3& lightDirection, const glm::vec3& lightColor, const HitInfo& hitInfo)
{
	...
	glm::vec3 avg = glm::normalize(cameraDirection + lightDirection);
	return (lightColor * hitInfo.material.ks * glm::pow(glm::dot(avg, hitInfo.normal), hitInfo.material.shininess)) + computeLambertianModel(state, cameraDirection, lightDirection, lightColor, hitInfo);
}
...
{
    Component c_low = this->components[0], c_high = this->components[this->components.size() - 1];
	...
	for(const auto & c : this->components) {
		if(c.t > c_low.t && c.t <= ti) {
			c_low = c;
		} else if (c.t < c_high.t && c.t >= ti) {
			c_high = c;
		}
	}
	float lerp_val = (ti - c_low.t) / (c_high.t - c_low.t);
	if(lerp_val < 0.0f) {
		return c_low.color;
	}
	if(lerp_val > 1.0f) {
	...
	return (lerp_val * c_high.color) + ((1.0f - lerp_val) * c_low.color);
}
...
glm::vec3 computeLinearGradientModel(RenderState& state, const glm::vec3& cameraDirection, const glm::vec3& lightDirection, const glm::vec3& lightColor, const HitInfo& hitInfo, const LinearGradient& gradient)
{
	...
	return lightColor * gradient.sample(cos_theta) * cos_theta;
}