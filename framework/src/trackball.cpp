...
void Trackball::setCamera(const glm::vec3 lookAt, const glm::vec3 rotations, const float dist)
{
	...
	m_rotation_angle = glm::quat(m_rotationEulerAngles);
}
...
Ray Trackball::generateRay(const glm::vec2& pixel) const
{
    ...
    Ray ray;
    ray.origin = position();
    ray.direction = m_rotation_angle * cameraSpaceDirection;
    ray.inv_d = 1.0f / ray.direction;
    ray.t = std::numeric_limits<float>::max();
    return ray;
}

glm::quat Trackball::get_rotation_quat() {
